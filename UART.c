...
void UART_Init(void){
 ...
 UART0_CTL_R &= ~0x00000001;      			// disable UART
  UART0_IBRD_R = 21;                    // IBRD = int(80,000,000 / (16 * 230400 )) = int(5.)
  UART0_FBRD_R = 45;                    // FBRD = round(0.425347222 * 64) = 26
  UART0_LCRH_R = (0x00000070);// 8 bit word length (no parity bits, one stop bit, FIFOs)
...
// Output: none
void UART_OutString(unsigned char buffer[]){int i=0;
...
void UART_ConvertUDec(unsigned long n){ long aux;
		...
		String[5] = '*';
				...
				String[0] = 0x30 + n/1000;          // thousands digit
				aux = n%1000;
				String[1] = 0x30 + aux/100;         // thousands digit
				aux = aux%100;
				String[2] = 0x30 + aux/10;          // thousands digit
				aux = aux%10;
				String[3] = 0x30 + aux;
			}
